...
// MARK:- Geometry
func intersection_line(start1: P, end1: P, start2: P, end2: P) -> Bool {
    
    let dir1 = P(x: end1.x - start1.x,y: end1.y - start1.y)
    let dir2 = P(x: end2.x - start2.x,y: end2.y - start2.y)
...
func pointInPolygon(point: P,points: [P], count: Int) -> Bool {
    var intersections: UInt = 0
    for i in 0...count-1 {
        let A = P(x: point.x,y: point.y)
        let B = P(x: point.x+pointInPolygonBeamSize,y: point.y)
        let C = points[i]
        var D = P()
        if(i != count-1) {
            D = points[i+1]
        ...
        intersections += UInt(intersection_line(A,end1: B,start2: C,end2: D))
    }
    return Bool(intersections & 1)
}
func inRange(firstPosition: P, secondPosition: P,  range: CGFloat) -> Bool {
...
func findAngleDirection(f: P, s: P) -> CGFloat {
    let dx = f.x - s.x
    let dy = f.y - s.y
    let dist2 = dx*dx + dy*dy
    let x = dx*dx/dist2
    let y = dy*dy/dist2
    return findAngle(V(dx: x, dy: y))
}
func findDirection(f: P, s: P) -> V {
    let dx = s.x - f.x
    let dy = s.y - f.y
    let dist2 = dx*dx + dy*dy
    let x = dx/sqrt(dist2)
...
func findDistanse(f: P, s: P) -> CGFloat {
    let dx = f.x - s.x
    let dy = f.y - s.y
    let dist2 = dx*dx + dy*dy
